...
import tqdm
import cv2
from typing import Callable, Union, Tuple, Sequence, Optional, Iterable, Dict, List
...
class TopdownAffine(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if self.use_udp:
            ...
            warp_mat_inverse = get_warp_matrix(center, scale, rot, output_size=(w, h), inv=True)
        ...
        if "next_img" in results:
            results["next_img"] = cv2.warpAffine(results['next_img'], warp_mat, warp_size, flags=cv2.INTER_LINEAR)        
        
        if "img_mask" in results:
            results["img_mask"] = cv2.warpAffine(results['img_mask'], warp_mat, warp_size, flags=cv2.INTER_NEAREST)
        
        if "next_img_mask" in results:
            results["next_img_mask"] = cv2.warpAffine(results['next_img_mask'], warp_mat, warp_size, flags=cv2.INTER_NEAREST)
        ...
        if results.get('prev_keypoints', None) is not None:
            transformed_keypoints = results['prev_keypoints'].copy()
            # Only transform (x, y) coordinates
            transformed_keypoints[..., :2] = cv2.transform(
                results['prev_keypoints'][..., :2], warp_mat)
            results['prev_transformed_keypoints'] = transformed_keypoints

        results['input_size'] = (w, h)
        results["trans_output"] = warp_mat
        results["trans_output_inverse"] = warp_mat_inverse
