...
class Diff_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "RGB" in self.network_input:
            prev_rgb_img = prev_rgb_img[bbox[0] : bbox[2], bbox[1] : bbox[3], :] # H x W x 3
            prev_mask_img = prev_mask_img[bbox[0] : bbox[2], bbox[1] : bbox[3], :] # H x W x 1
            prev_joints_2D_crop = np.zeros_like(prev_joints_2D)
            prev_joints_2D_crop[:, 0] = prev_joints_2D[:, 0] - bbox[1]
            prev_joints_2D_crop[:, 1] = prev_joints_2D[:, 1] - bbox[0]
            
            next_rgb_img = next_rgb_img[bbox[0] : bbox[2], bbox[1] : bbox[3], :] # H x W x 3
            next_mask_img = next_mask_img[bbox[0] : bbox[2], bbox[1] : bbox[3], :] # H x W x 1
            next_joints_2D_crop = np.zeros_like(next_joints_2D)
            next_joints_2D_crop[:, 0] = next_joints_2D[:, 0] - bbox[1]
            next_joints_2D_crop[:, 1] = next_joints_2D[:, 1] - bbox[0]            
        
        
            crop_h, crop_w = prev_rgb_img.shape[0], prev_rgb_img.shape[1]
            ...
            prev_rgb_img_input = _get_input(prev_rgb_img, trans_output, self.input_img_size[0], self.input_img_size[1], interpolation_method=cv2.INTER_LINEAR)
            prev_mask_img_input = _get_input(prev_mask_img, trans_output, self.input_img_size[0], self.input_img_size[1], interpolation_method=cv2.INTER_NEAREST)
            next_rgb_img_input = _get_input(next_rgb_img, trans_output, self.input_img_size[0], self.input_img_size[1], interpolation_method=cv2.INTER_LINEAR)
            next_mask_img_input = _get_input(next_mask_img, trans_output, self.input_img_size[0], self.input_img_size[1], interpolation_method=cv2.INTER_NEAREST)         
            
            prev_joints_2D_output = affine_transform_pts(prev_joints_2D_crop, trans_output)
            next_joints_2D_output = affine_transform_pts(next_joints_2D_crop, trans_output)
            ...
            if self.simcc_flag:
                next_joints_2D_output_visible = (next_joints_2D_output[:, 0] >= 0.0) * (next_joints_2D_output[:, 0] < self.output_img_size[0]) * (next_joints_2D_output[:, 1] >= 0) * (next_joints_2D_output[:, 1] < self.output_img_size[1])
                next_joints_2D_output_simcc = self.simcc_label.encode(keypoints=next_joints_2D_output[None, ...], keypoints_visible=next_joints_2D_output_visible[None, ...])
                
                keypoint_x_labels = next_joints_2D_output_simcc["keypoint_x_labels"].squeeze(0)
                keypoint_y_labels = next_joints_2D_output_simcc["keypoint_y_labels"].squeeze(0)
                keypoint_weights = next_joints_2D_output_simcc["keypoint_weights"].squeeze(0)
