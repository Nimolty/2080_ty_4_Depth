...
from depth_c2rp.utils.spdh_utils import apply_depth_normalization_16bit_image, heatmap_from_kpoints_array, gkern, compute_rigid_transform, compute_3n_loss
...
#    # 'l_gripper_l_finger',
#]
JOINT_NAMES = [f"panda_joint{idx}" for idx in range(14)
              ]
#JOINT_INFOS = 
#              {
...
def center_crop_depth_img(depth_img):
    # depth_img.shape == (H, W)
    H, W = depth_img.shape
    if H == W:
        return depth_img
        ...
        seq = (H - W) // 2
        return depth_img[seq : H - seq, :]
    else:
        seq = (W - H) // 2
        return depth_img[:, seq : W - seq]
...
class Depth_dataset(Dataset):
    ...
    def __getitem__(self, idx):    
        ...
        cx, cy = depth_img.shape[1] / 2 - 0.5, depth_img.shape[0] / 2 - 0.5
...
#        print("t2 - t1", t2 -t1)
        ...
        output = {
            ...
            'depth_img_raw' : torch.from_numpy(depth_img_raw[None, ...]),
            "depth_img_crop" : torch.from_numpy(depth_img_crop[None, ...]),
...
if __name__ == "__main__":
    PandaDataset = Depth_dataset(train_dataset_dir=Path("/DATA/disk1/hyperplane/Depth_C2RP/Data/Data_0201_test_syn/"),
                                            ...
                                            input_img_size=tuple([400, 400]),
                                            sigma=3.0,
                                            norm_type="min_max",
                                            network_input="XYZ",
                                            network_task="3d_RPE",
                                            depth_range=[500, 3500, 7.5],
    ...
    for idx, item in enumerate(PandaDataset):
        ...
        depth_img = item["depth_img_crop"].detach().cpu().numpy()[0]
        depth_img_raw = item["depth_img_raw"].detach().cpu().numpy()[0]
        cv2.imwrite(f"depth.png", item["depth"].detach().cpu().numpy().transpose(1,2,0) * 255)
        
        pts = depthmap2pointcloud(depth_img  / 1000, fx=558.11, fy=558.11, cx=199.5,
                                             cy=199.5)
        raw_pts = depthmap2pointcloud(depth_img_raw / 1000, fx=502.30, fy=502.30, cx=319.5,
                                             cy=179.5)
        np.savetxt(f"crop.txt", pts.reshape(-1,3))
        np.savetxt(f"raw.txt", raw_pts.reshape(-1,3))
        
        mask = item["mask"].detach().cpu().numpy()
        print(mask.shape)
        cv2.imwrite(f"mask.png", mask[:, :, None] * 255)
...
#        cv2.imwrite("visual_depth.png", item["depthvis"])
        ...
        img = cv2.imread(rgb_path)[:, 140 : 500, :]
        img = cv2.resize(img, tuple([400, 400]), interpolation=cv2.INTER_NEAREST)
...
#        
        img = mosaic_images(
                    images, rows=8, cols=3, inner_padding_px=10
                )
