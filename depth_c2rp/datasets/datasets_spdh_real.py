...
from depth_c2rp.utils.spdh_utils import apply_depth_normalization_16bit_image, heatmap_from_kpoints_array, gkern, compute_rigid_transform,pointcloud2depthmap, hole_filling
...
#    # 'l_gripper_l_finger',
#]
JOINT_NAMES = ["panda_joint" + str(idx+1) for idx in range(14)]
...
class Depth_dataset(Dataset):
    def __init__(self, train_dataset_dir: Path, val_dataset_dir : Path, joint_names: list, run: list, init_mode: str = 'train', img_type: str = 'D',
                 ...
                 aug_mode: bool = True, noise: bool = False, demo: bool = False, change_intrinsic: bool = False):
    ...
    def __getitem__(self, idx):    
        ...
        depth_img = cv2.imread(sample['depth_file'], cv2.IMREAD_UNCHANGED).astype(np.float32) # milimeters
        #depth_img[depth_img > 3380] = 0.0 
        #depth_img = np.clip(depth_img, 0.0, 3380)
        
        #print("max img", np.max(depth_img))
        ...
        if self.aug_mode:
            if self.aug_type == '2d':
                ...
                depth_img, joints_3D_Z, R2C_Mat_after_aug, R2C_Trans_after_aug, mask_file_res = augment_3d(intrinsic, points, depth_img, joints_3D_Z, R2C_Mat_before_aug, R2C_Trans_before_aug)
        ...
        if self.change_intrinsic:
            ...
            # Pay Attention to the cy!!!
            train_cx, train_cy = train_cam_settings_data["camera_settings"][0]["intrinsic_settings"]["cx"]* scale_x, train_cam_settings_data["camera_settings"][0]["intrinsic_settings"]["cy"]* scale_y - 12.0
            
            if self.depth_range_type == "padding":
                assert W > H
                train_cx = train_cx
                train_cy = train_cy + 96.0
                #cx = 191.7
                #cy = cx
...
if __name__ == "__main__":
    PandaDataset = Depth_dataset(train_dataset_dir=Path("/DATA/disk1/hyperplane/Depth_C2RP/Data/Real_Test/depth/"),
                                            val_dataset_dir=Path("/DATA/disk1/hyperplane/Depth_C2RP/Data/Real_Test/depth/"),
