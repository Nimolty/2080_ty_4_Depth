...
class Calculate_Loss(nn.Module):
    ...
    def forward(self, batch_dt_poses, batch_dt_joints_pos, batch_dt_masks, \
        ...
        if self.device is not None and self.cfg is not None and self.dr_engine == "Kaolin":
            ...
            self.DPRenderer.load_joints(batch_gt_joints_pos)
            batch_sample_pts_wrt_rob = self.DPRenderer.concat_and_sample_mesh(self.num_pts) # B x num_pts x 3

        if self.rt_loss_type == "entangle":
            batch_dt_res = torch.bmm(batch_dt_rot, batch_sample_pts_wrt_rob.permute(0, 2, 1).contiguous()) + batch_dt_trans[:, :, None] # (B, 3, num_pts) 
            batch_gt_res = torch.bmm(batch_gt_rot, batch_sample_pts_wrt_rob.permute(0, 2, 1).contiguous()) + batch_gt_trans[:, :, None] # (B, 3, num_pts) 
...
#        batch_compute_joints_wrt_cam = compute_concat_loss(batch_dt_rot, batch_dt_trans[:, :, None], batch_dt_joints_pos, batch_dt_rot.device)
#        if not self.is_res:
...
#        batch_check_2nplus2_wrt_cam = compute_2nplus2_loss(batch_gt_rot, batch_gt_trans[:, :, None], batch_gt_joints_pos, batch_gt_rot.device)
#        print(batch_check_2nplus2_wrt_cam.all() == batch_gt_2nplus2_wrt_cam.all())
