...
class NoiseScheduleSubVP:
    ...
    def marginal_std(self, t):
        ...
        Compute sigma_t of a given continuous-time label t in [0, T].
        """
        return 1. - torch.exp(2. * self.marginal_log_mean_coeff(t))
    ...
    def marginal_lambda(self, t):
        ...
        """
        log_mean_coeff = self.marginal_log_mean_coeff(t)
        log_std = torch.log(1. - torch.exp(2. * log_mean_coeff))
        return log_mean_coeff - log_std
    ...
    def inverse_lambda(self, lamb):
        ...
        """
        if self.schedule == 'linear':
            c = torch.log( (-torch.exp(-lamb) + torch.sqrt(torch.exp(-2 * lamb) + 4.)) / 2)
            Delta = (self.beta_0 ** 2 - 4 * c * (self.beta_1 - self.beta_0))
            return (-self.beta_0 + torch.sqrt(Delta)) / (self.beta_1 - self.beta_0)
            #tmp = 2. * (self.beta_1 - self.beta_0) * torch.logaddexp(-2. * lamb, torch.zeros((1,)).to(lamb))
            #Delta = self.beta_0**2 + tmp
            #return tmp / (torch.sqrt(Delta) + self.beta_0) / (self.beta_1 - self.beta_0)
        else:
