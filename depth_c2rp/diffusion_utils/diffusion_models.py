...
class ScoreModelFC_Adv(nn.Module):
    ...
    def __init__(self, config,
        ...
        self.cond_pose_mask_prob = config["DIFF_TRAINING"]["COND_POSE_MASK_PROB"]
        self.cond_part_mask_prob = config["DIFF_TRAINING"]["COND_PART_MASK_PROB"]
        self.cond_joint_mask_prob = config["DIFF_TRAINING"]["COND_JOINT_MASK_PROB"]
    ...
    def random_mask_condition(self, condition):
        ...
        # TODO: mask parts
        if self.cond_part_mask_prob > 0:
            final_mask = np.ones((batch_size, self.cond_n_joints), dtype=np.float32)
            mask = torch.bernoulli(
            ...
            condition = condition.view(batch_size, self.cond_n_joints, -1) * final_mask
        ...
        if self.cond_joint_mask_prob > 0:
            #print("joint mask !!!")
            mask = torch.bernoulli(
                torch.ones((batch_size, self.cond_n_joints, 1), device=condition.device) * self.cond_joint_mask_prob
            ) # 1-> use null condition, 0-> use real condition
            condition = condition.view(batch_size, self.cond_n_joints, -1) * (1. - mask)
            condition = condition.view(batch_size, -1)
