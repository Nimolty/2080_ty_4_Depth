...
from depth_c2rp.diffusion_utils.diffusion_utils_general import from_flattened_numpy, to_flattened_numpy, get_score_fn, get_noise_fn
...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps, device=None):
  ...
  sampler_name = config["DIFF_SAMPLING"]["METHOD"]
  # Probability flow ODE sampling with black-box ODE solvers
  if sampler_name.lower() == 'ode':
    sampling_fn = get_ode_sampler(sde=sde,
                                  shape=shape,
                                  inverse_scaler=inverse_scaler,
                                  denoise=config["DIFF_SAMPLING"]["NOISE_REMOVAL"],
                                  eps=eps,
                                  device=device)
  ...
  # Predictor-Corrector sampling. Predictor-only and Corrector-only samplers are special cases.
    ...
    corrector_name = config["DIFF_SAMPLING"]["CORRECTOR"].lower()
    ...
    sampling_fn = get_pc_sampler(sde=sde,
                                 ...
                                 corrector=corrector,
                                 inverse_scaler=inverse_scaler,
                                 snr=config["DIFF_SAMPLING"]["SNR"],
                                 n_steps=config["DIFF_SAMPLING"]["N_STEPS_EACH"],
                                 probability_flow=config["DIFF_SAMPLING"]["PROBABILITY_FLOW"],
                                 continuous=config["DIFF_TRAINING"]["CONTINUOUS"],
                                 denoise=config["DIFF_SAMPLING"]["NOISE_REMOVAL"],
                                 eps=eps,
                                 device=device)
...
class ReverseDiffusionPredictor(Predictor):
  ...
  def update_fn(self, x, t, mask):
    f, G = self.rsde.discretize(x, t, mask)
    z = torch.randn_like(x)
    x_mean = x - f
...
class AncestralSamplingPredictor(Predictor):
  ...
  def vesde_update_fn(self, x, t, mask):
    sde = self.sde
    timestep = (t * (sde.N - 1) / sde.T).long()
    sigma = sde.discrete_sigmas[timestep]
    adjacent_sigma = torch.where(timestep == 0, torch.zeros_like(t), sde.discrete_sigmas.to(t.device)[timestep - 1])
    score = self.score_fn(x, t, mask)
  ...
  def vpsde_update_fn(self, x, t, mask):
    sde = self.sde
    timestep = (t * (sde.N - 1) / sde.T).long()
  ...
  def update_fn(self, x, t,  mask):
    if isinstance(self.sde, sde_lib.VESDE):
      return self.vesde_update_fn(x, t, mask)
    elif isinstance(self.sde, sde_lib.VPSDE):
      return self.vpsde_update_fn(x, t, mask)
...
def shared_predictor_update_fn(x, t,  mask, sde, model, predictor, probability_flow, continuous):
...
def shared_corrector_update_fn(x, t, mask, sde, model, corrector, continuous, snr, n_steps):
  ...
  return corrector_obj.update_fn(x, t, mask)
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def get_imputation_update_fn(update_fn):
    ...
    def imputation_update_fn(x, vec_t, mask, model, args):
      x, x_mean = update_fn(x, vec_t, mask, model=model)
...
#        x_mean = x_mean * (1 - mask) + masked_data_mean * mask
  ...
  def pc_sampler(model,shape=shape, num_samples=None, denoise_x=None, args=None):
    ...
    with torch.no_grad():
      ...
      for i in range(sde.N):
        ...
        torch.cuda.synchronize()
        t2 = time.time()
        x, x_mean = corrector_imputation_update_fn(x, vec_t, mask=None, model=model, args=args)
        torch.cuda.synchronize()
        t3 = time.time()
        x, x_mean = projector_imputation_update_fn(x, vec_t, mask=None, model=model, args=args)
...
def get_ode_sampler(sde, shape, inverse_scaler,
  ...
  def denoise_update_fn(model, x, mask):
    ...
    _, x = predictor_obj.update_fn(x, vec_eps, mask)
    return x
  ...
  def drift_fn(model, x, t, mask):
    ...
    return rsde.sde(x, t, mask)[0]

  def ode_sampler(model, z=None, shape=shape, num_samples=None):
    ...
    with torch.no_grad():
      ...
      def ode_func(t, x):
        ...
        drift = drift_fn(model, x, vec_t, mask=None)
      ...
      if denoise:
        x = denoise_update_fn(model, x, mask=None)
...
def get_dpm_solver_sampler(sde, shape, inverse_scaler, steps=10, eps=1e-3,
  ...
  def dpm_solver_sampler(model, z=None, shape=shape):
    ...
    with torch.no_grad():
      ...
      noise_pred_fn = get_noise_fn(sde, model, mask=None, train=False, continuous=True)
...
def get_ddim_sampler(sde, shape, inverse_scaler, sampling_type, beta_schedule="linear", timesteps=10, num_timesteps=10,
    ...
    def ddim_sampler(model, shape=shape, num_samples=None):
        with torch.no_grad():
            # Initial sample
            noise_pred_fn = get_noise_fn(sde, model, mask=None, train=False, continuous=continuous)
            x = sde.prior_sampling(shape).to(device)
