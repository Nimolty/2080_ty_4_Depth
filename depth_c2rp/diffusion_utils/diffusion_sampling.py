...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model, condition, shape=shape, num_samples=None, denoise_x=None, args=None):
    ...
    with torch.no_grad():
      ...
      # make mask
      mask = torch.ones_like(condition)  # [B, j, 3], non-zero joints
...
def get_ode_sampler(sde, shape, inverse_scaler,
  ...
  def denoise_update_fn(model, x, condition, mask):
    score_fn = get_score_fn(sde, model, train=False, continuous=True)
    # Reverse diffusion predictor for denoising
    predictor_obj = ReverseDiffusionPredictor(sde, score_fn, probability_flow=False)
    vec_eps = torch.ones(x.shape[0], device=x.device) * eps
    _, x = predictor_obj.update_fn(x, vec_eps, condition, mask)
    return x

  def drift_fn(model, x, t, condition, mask):
    """Get the drift function of the reverse-time SDE."""
    score_fn = get_score_fn(sde, model, train=False, continuous=True)
    rsde = sde.reverse(score_fn, probability_flow=True)
    return rsde.sde(x, t, condition, mask)[0]

  def ode_sampler(model, condition, shape=shape, num_samples=None, z=None, track_idx=None):
    ...
    with torch.no_grad():
      ...
      mask = torch.ones_like(condition).to(device)  # [B, j, 3], non-zero joints
      ...
      def ode_func(t, x):
        ...
        drift = drift_fn(model, x, vec_t, condition, mask)
...
#                                     rtol=rtol, atol=atol, method=method)
      t1 = time.time()
#      solution = integrate.solve_ivp(ode_func, (0.004, 0.0001), to_flattened_numpy(x),
#                                     rtol=rtol, atol=atol, method=method)
      T = 0.004
      eps = 0.0001
      t_eval = np.linspace(T, eps, 2)
      solution = integrate.solve_ivp(ode_func, (T, eps), to_flattened_numpy(x),
      ...
      if denoise:
        x = denoise_update_fn(model, x, condition, mask)
...
def get_dpm_solver_sampler(sde, shape, inverse_scaler, steps=10, eps=1e-3,
  ...
  def dpm_solver_sampler(model, condition,shape=shape, num_samples=None, z=None):
...
def get_ddim_sampler(sde, shape, inverse_scaler, sampling_type, beta_schedule="linear", timesteps=10, num_timesteps=10, 
    ...
    def ddim_sampler(model, condition,shape=shape, num_samples=None, z=None, track_idx=0):
        with torch.no_grad():
            # Initial sample
            batch_size = condition.shape[0]
            defined_batch_size = shape[0]
            ...
            if track_idx == 0:
                ...
                betas = get_beta_schedule(beta_schedule, sde.beta_0 / sde.N, 0.001, 100)
                betas = torch.from_numpy(betas).float().to(device)
