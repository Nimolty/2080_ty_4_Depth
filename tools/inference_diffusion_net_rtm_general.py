...
#                                )
    ...
    rtm_model = torch.nn.parallel.DistributedDataParallel(rtm_model, device_ids=[cfg["LOCAL_RANK"]],
                                                output_device=cfg["LOCAL_RANK"],find_unused_parameters=False)
    ...
    if cfg["RESUME_HEATMAP"] != "":
        rtm_model.load_state_dict(torch.load(cfg["RESUME_HEATMAP"], map_location=device)["model"], strict=True)
...
#                    break
                ...
                if pred_2d_flag:
                    ...
                    joints_2d_pred = rtm_model.module.predict(input_tensor)
...
#            results_list = distributed_concat(torch.cat(results_list, dim=0), len(sampler.dataset))
            ...
            if pred_2d_flag:
                ...
                meta_json["uv_kps_pck_list"] = uv_kps_pck_list.tolist()
