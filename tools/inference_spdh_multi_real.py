...
from depth_c2rp.datasets.datasets_spdh_real import Depth_dataset
from depth_c2rp.build import build_whole_spdh_model
from depth_c2rp.utils.spdh_utils import load_spdh_model, write_prediction_and_gt, compute_kps_joints_loss, compute_3d_error, compute_3n_loss_42_cam, depthmap2points
...
def network_inference(model, cfg, epoch_id, device, mAP_thresh=[0.02, 0.11, 0.01], add_thresh=0.06,angles_thresh=[2.5, 30.0, 2.5]):
    ...
    test_sampler = SequentialDistributedSampler(test_dataset, batch_size=1)#eval_cfg["BATCH_SIZE"])
...
#            break 
        with torch.no_grad():
            ...
            if "/DATA/disk1/hyperplane/Depth_C2RP/Data/Real_Test/depth/000001.exr" in batch["depth_path"]:
                ...
                print("batch gt", batch["joints_2D_depth"][0])
            
#            print("2d uv", joints_2d_depth[0] - uv_pred[0])
#            print("input_K", input_K)
            
#            if dist.get_rank() == 0 and (batch_idx in visual_lst):
#                gt_images = batch['depthvis'].clone().numpy()
#                pred_images = gt_images.copy()
#                blend_images = gt_images.copy()
#                K = input_K.clone().detach().cpu().numpy()
...
#                gt_images, pred_images, true_blend_uv, true_blend_uz, pred_blend_uv, pred_blend_uz = get_blended_images(gt_images, K, joints_3d_gt_np, joints_3d_pred_np, heatmap_gt.device, heatmap_pred, heatmap_gt)
                
#                for b_idx in range(true_blend_uv.shape[0]):
#                    depth_path = depth_paths[b_idx] 
#                    depth_img = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
#                    pts = depthmap2points(depth_img, fx=910.4185791/2, fy=909.881958/2, cx=635.54394532/2, cy=359.45794678/2)
#                    pts = pts.reshape(-1, 3)
#                    np.savetxt(f"./pts/{str(batch_idx)}_{str(b_idx).zfill(2)}_sim.txt", pts / 1000)
#                    np.savetxt(f"./pts/{str(batch_idx)}_{str(b_idx).zfill(2)}_kps_pred.txt", joints_3d_pred_np[b_idx])
#                    np.savetxt(f"./pts/{str(batch_idx)}_{str(b_idx).zfill(2)}_kps_gt.txt", joints_3d_gt_np[b_idx])
                    
#                    cv2.imwrite(f"./imgs/{str(batch_idx)}_{str(b_idx).zfill(2)}_uv_gt.png", true_blend_uv[b_idx].transpose(1, 2, 0) * 255) 
#                    cv2.imwrite(f"./imgs/{str(batch_idx)}_{str(b_idx).zfill(2)}_uv_pred.png", pred_blend_uv[b_idx].transpose(1, 2, 0) * 255) 
#                    cv2.imwrite(f"./imgs/{str(batch_idx)}_{str(b_idx).zfill(2)}_uz_gt.png", true_blend_uz[b_idx].transpose(1, 2, 0) * 255) 
#                    cv2.imwrite(f"./imgs/{str(batch_idx)}_{str(b_idx).zfill(2)}_uz_pred.png", pred_blend_uz[b_idx].transpose(1, 2, 0) * 255) 
#                
#                for c_idx in range(c):
#                    cv2.imwrite(f"./imgs/{str(batch_idx)}_{str(c_idx).zfill(2)}_hm_gt.png", (heatmap_gt[0, c_idx:c_idx+1, :, :] * 255).permute(1,2,0).detach().cpu().numpy())
#                    cv2.imwrite(f"./imgs/{str(batch_idx)}_{str(c_idx).zfill(2)}_hm_pred.png", (heatmap_pred[0, c_idx:c_idx+1, :, :] * 255).permute(1,2,0).detach().cpu().numpy()) 
            ...
            for update_idx in range(dr_iter_num):
                ...
                if dist.get_rank() == 0 and (batch_idx in visual_lst):
                    for b_idx in range(render_depths.shape[0]):
                        ...
                        np.savetxt(f"./pts/{str(batch_idx)}_{str(b_idx).zfill(2)}_render.txt", this_render_depth)
                        np.savetxt(f"./pts/{str(batch_idx)}_{str(b_idx).zfill(2)}_kps_reconstruct.txt", reconstruct_pt)
...
#            pose_pred = pose_pred_clone
        ...
        #print(batch_add_from_pose(joints_kps_3d_pred.detach().cpu().numpy(), joints_kps_3d_gt.detach().cpu().numpy()))
...
#    ass_mAP_dict = dict()
    ...
    file_name = os.path.join(results_path, f"iter_{str(dr_iter_num)}_{testing_data_dir.split('/')[-2]}_{dr_loss}_{dr_input}_{dr_order}.txt")
