...
from depth_c2rp.utils.analysis import add_from_pose, add_metrics, print_to_screen_and_file
...
#                break
            ...
            for idx in range(batch_size):
                ...
                # compute add
                dt_trans, dt_quaternion, gt_joints_wrt_cam, gt_joints_wrt_rob = batch_dt_trans[idx], batch_dt_quaternion[idx], batch_gt_joints_wrt_cam[idx], batch_gt_joints_wrt_rob[idx]
                xyz_rp = batch_xyz_rp[idx][0]
                dt_quaternion /= np.linalg.norm(dt_quaternion)
                
                #print('dt_trans.shape', dt_trans.shape)
                #print("xyz.shape", xyz_rp.shape)
                
                add_mean = add_from_pose(dt_quaternion, dt_trans+xyz_rp, gt_joints_wrt_cam, gt_joints_wrt_rob)
                add.append(add_mean)
                
                # compute mAP
                this_mAP = []
                dt_joints_wrt_cam = batch_dt_joints_wrt_cam[idx] # N x 3
                # dist_3D = np.sqrt(np.sum((dt_joints_wrt_cam+xyz_rp - gt_joints_wrt_cam)**2, axis=-1,keepdims=True)) # N x 1
                if not is_res:
                    dist_3D = np.sqrt(np.sum((dt_joints_wrt_cam - gt_joints_wrt_cam)**2, axis=-1,keepdims=True)) # N x 1
                else:
                    d = dt_trans[2] + xyz_rp[2]
                    dt_joints_wrt_cam[:, 2] += d
                    dt_joints_wrt_cam[:, :2] = (dt_joints_wrt_cam[:, :2] + \
                                                     xyz_rp[:2] / xyz_rp[2]) * d 
                    dist_3D = np.sqrt(np.sum((dt_joints_wrt_cam - gt_joints_wrt_cam)**2, axis=-1,keepdims=True))
                                                     
                # print("dist_3D", dist_3D)
                for thres in thresholds:
                    avg_AP = np.sum(dist_3D < thres, axis=0) / dist_3D.shape[0]
