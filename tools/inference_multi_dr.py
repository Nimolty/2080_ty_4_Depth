...
from depth_c2rp.utils.utils import batch_quaternion_matrix, compute_concat_loss,exists_or_mkdir
...
#                np.savetxt(f"./check_depths/gt_{b}_xyz.txt", next_whole_xyz[b].permute(1,2,0).reshape(-1,3).detach().cpu().numpy())
            ...
            for update_idx in range(dr_iter_num):
                ...
                res = DPRenderer.loss_forward(next_whole_simdepth, batch_dt_mask, img_path=batch["next_frame_img_path"], update_idx=update_idx)
