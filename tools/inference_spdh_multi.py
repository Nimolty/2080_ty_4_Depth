...
#            for i in range(1000):
            t1 = time.time()
                #model(input_tensor, cam_params)
            heatmap_pred, joints_angle_pred, pose_pred, joints_3d_rob_pred, joints_3d_pred_norm, joints_3d_pred_tensor = model(input_tensor, cam_params, joints_1d_gt[..., None])
...
#                batch_acc_from_joint_angles(joints_angle_pred[:, link_idx-1:link_idx, :].detach().cpu().numpy(), joints_1d_gt[:, link_idx-1:link_idx, None].detach().cpu().numpy(), acc_thresholds)
                ...
                batch_add_from_pose(joints_kps_3d_pred.detach().cpu().numpy(), joints_kps_3d_gt.detach().cpu().numpy())
